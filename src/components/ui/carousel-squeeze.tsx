...
export type SqueezeCarouselProps = {
    ...
    height?: Size;
    /** Width/height of the photo, e.g. `16 / 9` (landscape) or `2 / 3` (portrait). Default `16 / 9`. */
    imageRatio?: number;
    /** How much room the open panel gives back to its neighbours at rest, as a fraction of the shared room. Default `-0.06`. Pass `0` for a photo-critical panel so it never crops at rest, only expanding further on hover. */
    heroIdleShare?: number;
    /** Same, but while a different panel is hovered. Default `-0.12`. */
    heroSqueezedShare?: number;
...
export function SqueezeCarousel({
    ...
    imageRatio = 16 / 9,
    heroIdleShare = BASE_SHARES[0],
    heroSqueezedShare = BASE_SQUEEZED[0],
    slatWidth = 8,
    slatGap = 8,
    gap = 16,
    radius = 6,
    ...
    const idleShares = [heroIdleShare, ...BASE_SHARES.slice(1)];
    const squeezedShares = [heroSqueezedShare, ...BASE_SQUEEZED.slice(1)];
    const shares = hoverGrow && hover >= 0 && hover <= 3 && !reduced ? null : idleShares;

    /** The share a column takes, once the pointer has had its say. */
    const shareOf = (col: number) => {
        if (shares) return shares[col];
        return hover === col ? STRETCHED[col] : squeezedShares[col];
    };
    ...
    const vars = {
        ...
        "--sq-hero": `calc(var(--sq-h) * ${imageRatio})`,
